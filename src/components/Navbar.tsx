        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              href={link.href}
              className={`transition-colors duration-300 font-medium text-base tracking-wide uppercase cursor-pointer ${
                scrolled || !isHome
          ...
          >
            <div className="flex flex-col items-center py-8 gap-6">
              ...
              <Link
                href="/contact"
                onClick={() => setIsOpen(false)}
                className="bg-white text-green-600 px-8 py-3 rounded-md font-bold w-3/4 shadow-md hover:bg-slate-100 transition-colors text-center cursor-pointer pointer-events-auto z-50"
              >
                Join Now
              </Link>
            </div>
